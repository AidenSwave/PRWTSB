...
type EditorMode="text"|"blueprint";
...
function AppShell(){
  ...
  const mutate=useCallback((next:Story)=>{setStory(syncStorySockets(next));setDirty(true)},[]);
  ...
  useEffect(()=>{const handler=(event:KeyboardEvent)=>{if((event.metaKey||event.ctrlKey)&&event.key.toLowerCase()==="s"){event.preventDefault();void save(event.shiftKey)}};window.addEventListener("keydown",handler);return()=>window.removeEventListener("keydown",handler)});
  useEffect(()=>bridge.onPreviewError(message=>setStatus(`Preview error: ${message}`)),[bridge]);
  return <main><header><div className="brand"><span className="mark">SE</span><div><strong>Script Editor</strong><small>{name}{dirty?" • Unsaved":""}</small></div></div><nav><button onClick={openFile}>Open</button><button disabled={!story} onClick={()=>save()}>Save</button><button disabled={!story} onClick={()=>save(true)}>Save As</button><span className="rule"/><button disabled={!story} onClick={addPassage}>+ Passage</button><button disabled={!story||mode!=="blueprint"} onClick={addGroup}>▧ Group</button><button disabled={!story} onClick={preview}>▶ Play</button><span className="rule"/><button className={mode==="text"?"active":""} disabled={!story} onClick={()=>setMode("text")}>Text</button><button className={mode==="blueprint"?"active":""} disabled={!story} onClick={()=>setMode("blueprint")}>Blueprint</button></nav></header>
    {story?(mode==="blueprint"?<BlueprintEditor story={story} onChange={mutate} onOpenPassage={id=>{setSelectedPassage(id);setMode("text")}}/>:<TextEditor story={story} selectedPassageId={selectedPassage} onSelectPassage={setSelectedPassage} onChangePassage={changePassage} onClose={()=>setMode("blueprint")}/>):<section className="map full"><div className="welcome"><div className="logo">SE</div><h1>Your story, without the machinery in the way.</h1><p>Open an exported Twine HTML file to edit its passages and blueprint.</p><button onClick={openFile}>Open story HTML</button></div></section>}
